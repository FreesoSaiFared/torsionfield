...
BRIDGE_CLIENT_SUFFIX = "-reboot-v4"
BRIDGE_FILES = ("reboot_bridge_v4.html", "reboot_bridge_v4.js")
...
def open_bridge(root: dict, extension: dict | None, source_dir: str | Path) -> dict:
    ...
    if not ensure_bridge_files(extension, source_dir):
        return {"ok": False, "reason": "bridge-v4-files-not-installed"}
    query = urllib.parse.urlencode({"client": root["client"], "server": bridge_base()})
    url = f"chrome-extension://{extension['id']}/reboot_bridge_v4.html?{query}"
...
def ensure_bridge(root: dict, source_dir: str | Path, timeout: float = 10) -> dict:
    ...
    return {
        "ok": False,
        "reason": "bridge-v4-page-did-not-register",
        "extension": extension,
        "url": opened.get("url"),
    }
...
def root_inventory(
    ...
    if bridge_healthy(root["client"]):
        try:
            # v4 may need to unfreeze/revive sleeping ChatGPT tabs before capture.
            # Allow enough root-level time for that bounded recovery, while every
            # individual page probe remains deadline-limited in the extension page.
            inventory = bridge_rpc(root["client"], "inventory", {}, 120_000)
            coverage = "extension"
        except Exception as exc:
    ...
    if inventory:
        ...
        if coverage == "extension":
            awaken_errors = ((inventory.get("awaken") or {}).get("errors") or [])
            for error in awaken_errors:
                errors.append("awaken:" + str(error))
